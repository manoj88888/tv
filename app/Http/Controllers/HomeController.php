...
class HomeController extends Controller
{
    public function checksubscription() {
        ...
        foreach ($all as $key => $value) {
            ...
            // dd($beforedays, $afterdays, $datetime1, $datetime2, $interval, $interval2);
            $url = url('account/purchaseplan');
            
            if($afterdays == 3 && $value->status == 0){
                /*fire a mail*/
    ...
    public function showall(Request $request, $menuid, $menuname)
    {
        ...
        foreach ($menu_items as $value) {
            ...
            } elseif($value->tv_series_id != null) {
            ...
            } elseif($value->audio_id != null) {
                $items->push(Audio::where('id', $value->audio_id)->first());
            }
        }
    ...
    public function guestindex(Request $request, $menu_slug)
    {
        ...
        if ($config->prime_genre_slider == 1) {
            //Layout 1
            return view('home', compact('getallgenre', 'menu_data', 'recent_data', 'ad', 'sliderview', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'home_blocks', 'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age','audio'));
        } else {
            //Layout 2
            return view('home2', compact('getallgenre', 'menu_data', 'recent_data', 'ad', 'sliderview', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'home_blocks', 'audiolanguages', 'section6', 'blogs', 'liveevent', 'home_slides', 'age','audio'));
        }
    ...
    public function index(Request $request, $menu_slug)
    {
        ...
        $movie = Movie::orderby('id', 'desc')->where('status', 1)->where('live', 0)->get();
        $program = TvSeries::select('tv_series.id as tv_series_ids', 'tv_series.keyword', 'tv_series.description', 'tv_series.title', 'tv_series.tmdb_id', 'tv_series.tmdb', 'tv_series.fetch_by', 'tv_series.thumbnail', 'tv_series.poster', 'tv_series.genre_id', 'tv_series.detail', 'tv_series.rating', 'tv_series.episode_runtime', 'tv_series.maturity_rating', 'tv_series.featured', 'tv_series.type', 'tv_series.status', 'tv_series.created_by', 'seasons.id as seasonidforplaynow', 'seasons.season_slug', 'seasons.trailer_url')
                            ->leftJoin('seasons', 'seasons.tv_series_id', '=', 'tv_series.id')
                            ->where('tv_series.status', 1)
                            ->groupBy('tv_series_ids')
                            ->get();
                        // dd($program);
        // $program = TvSeries::where('tv_series.status', 1)->get();
         $seasonslug = Season::all();
        // $tvshows = Movie::select('movies.id as movies_id','movies.tmdb_id', 'movies.title', 'movies.slug', 'movies.keyword', 'movies.description', 'movies.duration', 'movies.thumbnail', 'movies.poster', 'movies.tmdb', 'movies.fetch_by', 'movies.director_id', 'movies.actor_id', 'movies.genre_id', 'movies.trailer_url','movies.detail', 'movies.rating', 'movies.maturity_rating','movies.subtitle', 'movies.publish_year', 'movies.released', 'movies.upload_video', 'movies.featured', 'movies.series', 'movies.a_language', 'movies.audio_files', 'movies.type', 'movies.live', 'movies.livetvicon', 'movies.status', 'movies.is_protect', 'movies.password', 'movies.created_by', 'videolinks.id', 'videolinks.movie_id', 'videolinks.ready_url')
        //                     ->where('movies.live', 1)
        //                     ->orderBy('movies.id', 'DESC')
        //                     ->join('videolinks', 'videolinks.movie_id', '=', 'movies.id')
        //                     ->get();
        $tvshows = Movie::where('live', 1)->get();
        $audio = Audio::orderby('audio.id', 'desc')->get();
        ...
        if ($config->prime_genre_slider == 1) {
            //Layout 1
            return view('home', compact('menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'home_blocks', 'audiolanguages', 'section6', 'getallgenre', 'blogs', 'liveevent','protip','audio', 'movie', 'program', 'tvshows', 'seasonslug'));
        } else {
            //Layout 2
            return view('home2', compact('menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'home_blocks', 'audiolanguages', 'section6', 'getallgenre', 'blogs', 'liveevent','protip','audio', 'movie', 'program', 'tvshows', 'seasonslug'));
        }
    ...
    public function showallgenre(Request $request, $id)
    {
        ...
        if (isset($genre)) {
            $items = collect();
            $movies = Movie::where('genre_id', 'REGEXP', '(^|,)' . $genre->id . '(,|$)')->where('status', 1)->get();

            foreach ($movies as $movie) {
                $items->push($movie);
            }

            $tvs = TvSeries::where('genre_id', 'REGEXP', '(^|,)' . $genre->id . '(,|$)')->where('status', 1)->get();
